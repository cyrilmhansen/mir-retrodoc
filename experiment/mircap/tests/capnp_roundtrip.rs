...
fn check_fixture_roundtrip(name: &str) {
    ...
    assert!(
        orig_validation.is_ok(),
        "Original fixture {} must be valid: {:?}",
        name,
        orig_validation
    );
    ...
    assert!(
        !capnp_bytes.is_empty(),
        "Serialized capnp bytes must not be empty"
    );
    ...
    assert_eq!(
        original, decoded,
        "Decoded ModuleImage must be logically equal to original for fixture {}",
        name
    );
    ...
    assert!(
        dec_validation.is_ok(),
        "Decoded fixture {} must be valid: {:?}",
        name,
        dec_validation
    );
