...
"#;
    ...
    let err = runner.run_entry_by_name("main", &[]).expect_err("trap expected");
    assert!(matches!(err, ExecutionError::Trap(ExecutionTrap::ExplicitTrap { .. })));
    assert!(matches!(runner.trace_snapshot().outcome, mirsem::trace::TraceOutcome::Trapped(ExecutionTrap::ExplicitTrap { .. })));
}
...
insn 1 branch b:1
"#;
    let profile = ExecutionProfile { max_instructions: 8, ..ExecutionProfile::default() };
    let mut runner = runner_from_text(text, profile);
    let err = runner.run_entry_by_name("main", &[]).expect_err("fuel trap expected");
    assert!(matches!(err, ExecutionError::Trap(ExecutionTrap::FuelExhausted { max_instructions: 8 })));
}
...
insn 2 ret v:0
"#;
    let profile = ExecutionProfile { max_call_depth: 4, ..ExecutionProfile::default() };
    let mut runner = runner_from_text(text, profile);
    let err = runner.run_entry_by_name("main", &[]).expect_err("stack trap expected");
    assert!(matches!(err, ExecutionError::Trap(ExecutionTrap::StackOverflow { max_depth: 4 })));
}
...
insn 2 ret v:0
"#;
    let profile = ExecutionProfile { linear_memory_size: 128, stack_size: 64, ..ExecutionProfile::default() };
    let mut runner = runner_from_text(text, profile);
    let err = runner.run_entry_by_name("main", &[]).expect_err("heap/stack trap expected");
    assert!(matches!(err, ExecutionError::Trap(ExecutionTrap::HeapStackCollision { .. })));
}
...
"#;
    ...
    let mut runner = runner_from_text(text, profile);
    let err = runner.run_entry_by_name("main", &[]).expect_err("misaligned trap expected");
    assert!(matches!(err, ExecutionError::Trap(ExecutionTrap::MisalignedLoad { .. })));
}
