...
fn show_help() -> String {
    ...
    s.push_str(
        "  cost <input_file>                         Prints symbolic lowered-plan cost summaries.\n",
    );
    s.push_str(
        "  trace-cost <input_file>                   Compares symbolic cost summaries with a mirsem trace.\n",
    );
...
fn parse_args() -> Result<Args, String> {
    ...
    let (input, output) = match command.as_str() {
        "validate" | "decode" | "dump" | "run" | "analyze" | "trace-check" | "cost"
        | "trace-cost" | "plan" | "lower" | "bench-load" | "diff" | "diff-upstream"
        | "diff-rv32i" => {
...
fn main() {
    ...
    let result = match args.command.as_str() {
        ...
        "cost" => commands::cmd_cost(&args.input, args.format.as_deref(), args.json),
        "trace-cost" => {
            commands::cmd_trace_cost(&args.input, args.format.as_deref(), entry_name, args.json)
        }
