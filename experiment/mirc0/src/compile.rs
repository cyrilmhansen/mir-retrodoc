use crate::c_emit::{emit_instruction, emit_type};
use crate::error::CompileError;
use crate::pretty::pretty_print_c;
use crate::runtime_c::RUNTIME_HEADER;
use mircap::{Function, ModuleImage, SymbolKind, TypeKind};
...
pub fn compile(image: &ModuleImage, entry_name: &str) -> Result<String, CompileError> {
    ...
    let entry_ret_type = if entry_func.results.is_empty() {
        ...
        image
            .type_kind(entry_func.results[0])
...
fn find_entry_function<'a>(
    image: &'a ModuleImage,
    name: &str,
) -> Result<&'a Function, CompileError> {
    image
        .functions
        .iter()
        .find(|f| {
...
fn emit_function_declaration(image: &ModuleImage, func: &Function) -> Result<String, CompileError> {
    let ret_str = if func.results.is_empty() {
        ...
        let kind = image
            .type_kind(func.results[0])
            .ok_or(CompileError::UnsupportedType(TypeKind::Void))?;
        emit_type(kind)?
    } else {
...
fn emit_function_implementation(
    ...
    for i in func.params.len()..(func.value_count as usize) {
        ...
        if kind != TypeKind::Void {
            ...
            let init_val = if matches!(kind, TypeKind::I32) {
                "0"
            } else {
                "0u"
            };
    ...
    for &block_id in &func.blocks {
        let block = image
            .block(block_id)
            .ok_or_else(|| CompileError::Validation(vec![]))?;
